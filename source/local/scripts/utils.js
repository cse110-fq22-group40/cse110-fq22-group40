/**
 * This file contains the backend utility functions for update methods
 * and other miscellaneous functions for front end.
 */
 import {
  dict_typeFs,
  folder_utils,
 ...
 * Ex. update_name("F Major scales", "Bob", "1/11 Practice", "G Major Scales")
 */
 ...
 *
 * @usage
 * Ex. update_timestamp("Bob", "1/11 Practice", "G Major Scales", 3600, 1250)
 */
export function update_timestamp(typeFName, typeAName, audioObjName, 
  timestamp, newTimestamp) {
  if (
    !dict_typeFs[typeFName].get_typeA(
      typeAName.get_audio(audioObjName).does_note_exist(timestamp)
    )
  )
    throw new Error(
      `Note with the old timestamp "${timestamp}" does not exist`
    );

  if (
    dict_typeFs[typeFName].get_typeA(
      typeAName.get_audio(audioObjName).does_note_exist(newTimestamp)
    )
  )
    throw new Error(
      `Note with the new timestamp "${newTimestamp}" already exists`
    );
  ...
  Object.keys(localStorage).forEach((typeFName) => {
    ...
    for (const typeAName in typeF.dict_typeA) {
      ...
      for (const audioName in typeA.dict_audio) {
        audio_utils.add_audio(
          typeFName,
          typeAName,
          audioName,
          typeA.dict_audio[audioName].path,
          false
        );
        ...
        for (const timestamp in audioObj.notes) {
          notes_utils.add_note(
            typeFName,
            typeAName,
            audioName,
            timestamp,
            audioObj.notes[timestamp],
            false
          );
        }
      }
